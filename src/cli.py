...
@app.command()
def run(
    config: Path = typer.Argument(..., help="Path to experiment configuration YAML file"),
    dry_run: bool = typer.Option(False, "--dry-run", "-d", help="Validate config and show summary without running"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
    force: bool = typer.Option(False, "--force", "-f", help="Force run even if cache exists")
) -> None:
    ...
    try:
        ...
        if dry_run:
            ...
            console.print("[yellow]Dry run completed. Use --no-dry-run to actually run the experiment.[/yellow]")
            return
...
@app.command()
def list_configs(
    configs_dir: Path = typer.Option(Path("configs"), "--dir", "-d", help="Directory to search for configs")
) -> None:
...
@app.command()
def results(
    experiment_name: Optional[str] = typer.Argument(None, help="Experiment name to show results for"),
    cache_dir: Path = typer.Option(Path("cache"), "--cache-dir", "-c", help="Cache directory to search"),
    list_all: bool = typer.Option(False, "--list", "-l", help="List all available experiments")
) -> None:
...
@app.command()
def clean(
    cache_dir: Path = typer.Option(Path("cache"), "--cache-dir", "-c", help="Cache directory to clean"),
    ...
    if not force:
        confirm = typer.confirm(f"This will delete old cache files in {cache_dir}. Continue?")
        if not confirm:
...
def _show_config_summary(config: ExperimentConfig) -> None:
    ...
    # Experiment info
    table.add_row("Experiment", f"Name: {config.experiment.name}\nSeed: {config.experiment.random_seed}")
...
def _list_all_experiments(result_files: List[Path], run_files: List[Path]) -> None:
    ...
    for run_file in sorted(run_files, key=lambda x: x.stat().st_mtime, reverse=True):
        try:
            ...
            model_name = run_data.get("config", {}).get("model", {}).get("name", "Unknown")
            test_acc = run_data.get("results", {}).get("test_metrics", {}).get("accuracy", "N/A")
...
def _show_experiment_results(name: str, result_files: List[Path], run_files: List[Path]) -> None:
    ...
    if "config" in run_data:
        ...
        console.print(f"[blue]Model:[/blue] {config_dict.get('model', {}).get('name', 'Unknown')}")
        console.print(f"[blue]Epochs:[/blue] {config_dict.get('training', {}).get('epochs', 'Unknown')}")
