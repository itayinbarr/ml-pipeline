...
class DataConfig(pd.BaseModel):
    ...
    def validate_batch_size(cls, v: int) -> int:
        ...
        if v > 1024:
            raise ValueError("batch_size too large (>1024), consider reducing for memory")
        return v
...
class LinearModel(pd.BaseModel):
    ...
    input_size: int = Field(784, gt=0, description="Input feature dimension (28x28=784 for MNIST)")
...
class MLPModel(pd.BaseModel):
    ...
    activation: Literal["relu", "tanh", "sigmoid", "gelu"] = Field("relu", description="Activation function")
...
class CNNModel(pd.BaseModel):
    ...
    input_channels: int = Field(1, gt=0, description="Number of input channels (1 for grayscale MNIST)")
    num_classes: int = Field(10, gt=0, description="Number of output classes")
    channels: List[int] = Field([32, 64], min_length=1, max_length=5, description="Channel sizes for conv layers")
...
class SchedulerConfig(pd.BaseModel):
    ...
    factor: float = Field(0.5, gt=0.0, lt=1.0, description="Factor by which to reduce LR")
    patience: int = Field(5, gt=0, description="Epochs with no improvement after which LR is reduced")
...
class EarlyStoppingConfig(pd.BaseModel):
    ...
    patience: int = Field(5, gt=0, description="Epochs with no improvement after which training stops")
    min_delta: float = Field(0.001, ge=0.0, description="Minimum change to qualify as improvement")
...
class TrainingConfig(pd.BaseModel):
    ...
    optimizer: Literal["adam", "sgd", "adamw", "rmsprop"] = Field("adam", description="Optimizer type")
    weight_decay: float = Field(0.0, ge=0.0, le=0.1, description="L2 regularization strength")
    momentum: float = Field(0.9, ge=0.0, lt=1.0, description="Momentum for SGD optimizer")
    scheduler: Optional[SchedulerConfig] = Field(None, description="Learning rate scheduler")
    early_stopping: Optional[EarlyStoppingConfig] = Field(None, description="Early stopping configuration")

    @pd.field_validator("learning_rate")
    @classmethod
    def validate_learning_rate(cls, v: float) -> float:
        if v > 0.1:
            raise ValueError("learning_rate >0.1 is very high and may cause instability")
        return v
...
class EvaluationConfig(pd.BaseModel):
    ...
    metrics: List[Literal["accuracy", "f1_macro", "f1_micro", "precision", "recall"]] = Field(
        ["accuracy"], min_length=1, description="Metrics to compute"
    )
    save_predictions: bool = Field(False, description="Whether to save model predictions")
    save_confusion_matrix: bool = Field(True, description="Whether to save confusion matrix")
    save_per_class_metrics: bool = Field(False, description="Whether to save per-class metrics")
...
class DataAugmentationConfig(pd.BaseModel):
    ...
    rotation_degrees: float = Field(10.0, ge=0.0, le=45.0, description="Max rotation in degrees")
    translation: float = Field(0.1, ge=0.0, le=0.5, description="Max translation as fraction")
    scale: List[float] = Field([0.9, 1.1], min_length=2, max_length=2, description="Scale range [min, max]")
...
class RegularizationConfig(pd.BaseModel):
    ...
    label_smoothing: float = Field(0.0, ge=0.0, le=0.5, description="Label smoothing factor")
...
class ResourceConfig(pd.BaseModel):
    ...
    max_memory_gb: int = Field(8, gt=0, le=128, description="Maximum memory usage in GB")
    distributed: bool = Field(False, description="Whether to use distributed training")
    mixed_precision: bool = Field(False, description="Whether to use mixed precision training")
...
class ExperimentConfig(pd.BaseModel):
    ...
    def validate_experiment_consistency(self) -> "ExperimentConfig":
        ...
        if self.training.epochs < 2 and self.evaluation.save_per_class_metrics:
            raise ValueError("Need at least 2 epochs to compute meaningful per-class metrics")
            
        # Validate scheduler-optimizer compatibility
        if (self.training.scheduler and 
            self.training.scheduler.name == "reduce_lr_on_plateau" and
            self.training.optimizer == "sgd" and
            self.training.momentum == 0.0):
            raise ValueError("SGD with reduce_lr_on_plateau typically needs momentum > 0")
...
class ExperimentMetadata(pd.BaseModel):
    ...
    device: Literal["auto", "cpu", "cuda", "mps"] = Field("auto", description="Device to use for training")
    ...
    save_best_only: bool = Field(True, description="Whether to save only the best model")
