...
class MNISTDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f"Loaded MNIST {'train' if train else 'test'} set: {len(self.dataset)} samples")
...
def create_transforms(
    ...
    base_transforms = []
    if normalize:
        base_transforms.append(transforms.Normalize((0.1307,), (0.3081,)))  # MNIST stats
    ...
    if augmentation_config and augmentation_config.enabled:
        ...
        if augmentation_config.translation > 0:
            train_transform_list.append(
                transforms.RandomAffine(
                    degrees=0, 
                    translate=(augmentation_config.translation, augmentation_config.translation)
                )
            )
        ...
        if augmentation_config.scale != [1.0, 1.0]:
            train_transform_list.append(
                transforms.RandomAffine(
                    degrees=0, 
                    scale=tuple(augmentation_config.scale)
                )
            )
    ...
    logger.info(f"Created transforms - Train: {len(train_transform_list)}, Test: {len(base_transforms)}")
...
def create_datasets(
    ...
    logger.info(f"Dataset sizes - Train: {len(train_dataset)}, Val: {len(val_dataset) if val_dataset else 0}, Test: {len(test_dataset)}")
...
def get_dataset_stats(datasets: Tuple[Dataset, Optional[Dataset], Dataset]) -> Dict[str, Dict[str, float]]:
...
def validate_data_shape(dataloaders: Dict[str, DataLoader], expected_shape: Tuple[int, ...]) -> bool:
