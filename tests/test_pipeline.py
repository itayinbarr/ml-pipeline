...
class TestExperiment:
    ...
    def test_create_model(self, mock_create_model):
        ...
        with patch('src.experiment.pipeline.create_infra'):
            with patch('src.experiment.pipeline.get_model_summary', return_value="Model summary"):
                experiment = Experiment(config)
    ...
    def test_create_optimizer_and_scheduler(self):
        ...
        with patch('src.experiment.pipeline.create_infra'):
            with patch('src.experiment.pipeline.create_optimizer') as mock_create_opt:
                with patch('src.experiment.pipeline.create_scheduler') as mock_create_sched:
                    mock_optimizer = Mock()
                    mock_scheduler = Mock()
    ...
    def test_create_optimizer_without_model_raises_error(self):
        ...
        with patch('src.experiment.pipeline.create_infra'):
            ...
            with pytest.raises(ValueError, match="Model must be created before optimizer"):
                experiment.create_optimizer_and_scheduler()
...
class TestExperimentRun:
    ...
    def test_experiment_run_success(self, mock_scheduler, mock_optimizer_fn, mock_model_fn, mock_prepare_data):
    ...
    def test_experiment_run_with_error(self, mock_scheduler, mock_optimizer_fn, mock_model_fn, mock_prepare_data):
    ...
    def test_cached_data_preparation(self):
        ...
        with patch('src.experiment.pipeline.create_infra') as mock_create_infra:
            ...
            with patch('src.experiment.pipeline.prepare_data') as mock_prepare_data:
                with patch('src.experiment.pipeline.validate_data_shape', return_value=True):
                    mock_prepare_data.return_value = ({"train": Mock()}, {"stats": "test"})
                    ...
                    mock_infra.save_artifact.assert_called_with({"stats": "test"}, "dataset_statistics")
